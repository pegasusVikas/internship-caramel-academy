require('../models/user/instructor.model');
		...
		if (!err) {
			try {
                ...
                const msg = {
                    ...
                    text: 'You have successfully registered as an Instructor at Caramel IT Academy!',
                };
                sgMail.send(msg);
            } catch (err) {
                console.log(err.message);
            }
			res.json({doc:doc,registered:true});
		}		
		else {
			if (err.code == 11000)
...
module.exports.authenticate = (req, res, next) => {
	...
	passport.authenticate('instructorLocal', (err, instructor, info) => {
		...
		if (instructor) return res.status(200).json({ "token": instructor.generateJwt(), logged:true,instructor:instructor });
