...
const convert_lesson = () => {
  mammoth
    .convertToHtml({ path: "./uploads/lesson.docx" })
...
const convert_scenarios = () => {
  mammoth
    .convertToHtml({ path: "./uploads/scenario.docx" })
    .then(function (result) {
      const html = result.value; // The generated HTML
      const converted = tabletojson.convert(html, {
        useFirstRowForHeadings: true,
      ...
      var reswe = converted[0];
      fs.writeFile("data/scenarios.json", JSON.stringify(reswe), function (
        err
      ) {
        if (err) throw err;
...
const convert_projects = () => {
  mammoth
    .convertToHtml({ path: "./uploads/project.docx" })
    .then(function (result) {
      const html = result.value; // The generated HTML
      const converted = tabletojson.convert(html, {
        useFirstRowForHeadings: true,
